...
bool CannonLayer::init()
{
	if(CCLayer::init()){
		...
		_weapon->setPosition(winSize.width/2, _weapon->getCannon()->getCurCannonSprite()->getContentSize().height*0.26);
		...
		menu->setPosition(CCPointMake(winSize.width/2, _subItem->getContentSize().height/2));
		this->addChild(menu);
		return true;
	}
	return false;
